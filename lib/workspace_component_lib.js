...
function _get_component_result(componentId, processId) {
  return new Promise((resolve, reject) => {
    ...
    promises.push(new Promise((resolve, reject) => {
      historiqueStartModel.find({
          processId: processId,
          componentId: componentId
        })
        .lean()
        .exec((err, historiqueStart) => {
          if (err) {
            reject(err);
          } else {
            resolve(historiqueStart);
          }
        })
    }));
    Promise.all(promises).then(historiques => {
      let out = {};
      out.end = historiques[0];
      out.start = historiques[1];
      resolve(out);
    }).catch(e=>{
      reject(e);
    })
