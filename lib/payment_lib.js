var stripe = require('stripe')("sk_live_nNb1PXkgbWWFGq2sNpKy4Mrk" || 'pk_test_SzHbTFS2RaTIzBiKLvCvWGGz');
var user_lib = require('./user_lib')
var secret_stripe = require('../../../configuration').secret_stripe
      ...
      if (user.active) {
        ...
        if (user_infos.card.card.three_d_secure == "required" || user_infos.card.card.three_d_secure == "optional") {
          stripe.sources.create({
            ...
            redirect: {
              return_url: "https://semantic-bus.org/ihm/application.html?amount=" + amount + "#profil//addCredit"
            }
    ...
    user_lib.get({
      ...
      if (secret == user.secret_stripe) {
        stripe.charges.create({
          ...
          user_lib.update(user).then((lastUser) => {
...
//     })
//   }
//   console.log("NEW AMOUT 2", parseInt(amount) + (parseInt(amount) * parseInt(user_update.data.discount)))
//   user_update.data.credit += parseInt(amount) + (parseInt(amount) * user_update.data.discount)
