...
function _update_mainprocess(preData) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    if (preData.hash_password) {
      console.log("HASHED NEW PASSWORD", preData.hash_password)
      if (!toUpdate['$set']) {
        toUpdate['$set'] = {};
      }
...
function _update_preprocess(userParams, mailChange) {
  ...
  return new Promise(function (resolve, reject) {
    if (config.quietLog != true) {
      console.log("update preprocess : userParams || ", "mailChange : ", userParams);
    }
    if (mailChange) {
      userModel.findOne({
          ...
          var hash_password = new Promise(function (resolve, reject) {
            if (userParams.new_password) {
              _hash_password(userParams.new_password).then(function (hashedPassword) {
                ...
                reject({err: "bad_format_password"})
              })
            }else{
              resolve(null)
            }
      ...
      Promise.all([job, society, workspace, name, active, resetpasswordtoken, resetpasswordmdp, hash_password]).then(function (user_update_data) {
        var o = {};
