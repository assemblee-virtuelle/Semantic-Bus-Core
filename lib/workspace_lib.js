...
module.exports = {
  ...
  cleanHoldProcess: _cleanHoldProcess
};
...
function _cleanHoldProcess(workflow) {
  return new Promise((resolve, reject) => {
    ...
    processModel
      ...
      .then(processes => {
        ...
        historiqueEndModel.find({
          ...
          .exec()
          .then(historicsToClean => {
            console.log(historicsToClean);
            historicsToClean.forEach(historicToClean => {
              historicToClean.data = undefined;
              historicToClean.save();
            })
