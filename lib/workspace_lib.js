...
function _update_mainprocess(preData) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    //     toUpdate['$set'] = {};
    //   }
    //   toUpdate['$set']['components'] = preData.components.map(comp => {
    //     return comp._id
    //   });
    // }
    ...
    //   new: true
    // }, function(err, worspaceData) {
    //   if (err) {
    //     reject(err)
    //   } else {
    //     resolve(worspaceData)
    //   }
    // });
  })
} // <= _update_mainprocess
        ...
        if (err) {
          ...
          componentsPromises = componentsPromises.concat(
            sift({
              ...
              c.idString = c._id.toString(); //need because objetId don't exist for sift
              return c;
            })).map(c => {
