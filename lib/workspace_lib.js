...
var workspaceGraphTraitement = require('../../../utils/graph-traitment')
  ...
  return new Promise(function (resolve, reject) {
    ...
    userModel.findOne({
      ...
      .exec((error, data) => {
        ...
        let workspaces = sift({
          role: role
        }, data.workspaces).map(r => r.workspace);
        resolve(workspaces);
      })
...
function _update(workspace) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    _update_mainprocess(workspace).then(function (data) {
      resolve(data)
    }).catch(e => {
      reject(e);
    });
...
function _get_workspace_graph_data(workspace_id) {
  if (config.quietLog != true) {
    //console.log("=============== getworkspace ===========", workspace_id)
  }
  return new Promise(function (resolve, reject) {
    workspaceModel.findOne({
      ...
      .populate({
        path: 'components',
        select: 'consumption_history pricing module name'
      })
      .lean()
      .exec(function (err, workspace) {
        if (err) {
          ...
          //   return c;
          // })
          workspaceGraphTraitement.formatDataWorkspaceGraph(workspace).then((workspaceGraph)=>{
            resolve(workspaceGraph);
          })
        }
      })
  })
